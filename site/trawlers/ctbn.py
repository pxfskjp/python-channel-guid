...
URL="http://www.tbn.tv/watch-us/?date="

def get_tbn_date(the_date):
	...
	table = soup.findAll('table', attrs={'class':'broadcast-schedule-tbl'})[1]
	...
	for row in rows:
	    cols = row.findAll('td')
	    ptime = datetime.datetime.combine(the_date, datetime.datetime.strptime( cols[0].text.split("&")[0]  ,  "%I:%M %p"    ).time())
...
class TrawlerTBN(Trawler):
	@staticmethod
	def get_info_for_days(days):
		schedule = {}
		for day in days:
			schedule.update( {day.strftime("%Y-%m-%d"): get_tbn_date(day)})
