...
function add_days () {
	...
	$.ajax({
		url: "proxy.php?url=http://localhost:8080/days",
		cache: false
	}).done(function(data) {
		var is_first = false;
		// alert(html);
...
function populate_channel(chan, dom) {
	...
	$.ajax({
		url: "http://www.eternityready.com/channel-guide/proxy.php?url=http://localhost:8080/schedule/" + chan.number + "/" + GLOBAL_selected_date,
		//cache: false
	}).done(function(data) {
		if (data["error"] == "yes") {
...
function add_channels() {
	...
	$.ajax({
		url: "/channel-guide/proxy.php?url=http://localhost:8080/channels",  
		cache: true
	}).done(function(data) {
